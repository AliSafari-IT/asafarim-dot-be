...
<body>
    ...
    public async Task SendPasswordResetEmailAsync(string email, string magicLink)
    {
        try
        {
            // Validate SMTP configuration
            ...
            )
            {
                ...
                _logger.LogInformation(
                    "📧 Password Reset Email (Logged) - To: {Email}, Magic Link: {Link}",
                    email,
                    magicLink
                );
                return;
            ...
            message.From.Add(MailboxAddress.Parse(fromAddress));
            message.To.Add(MailboxAddress.Parse(email));
            message.Subject = "Reset Your Password - Asafarim";
...
<body>
    <div class='container'>
        ...
        <div class='content'>
            ...
            <center>
                <a href='{magicLink}' class='button'>Reset Password</a>
            </center>
            <p>Or copy and paste this link in your browser:</p>
            <div class='link-text'><a href='{magicLink}'>{magicLink}</a></div>
            <p style='color: #999; font-size: 12px;'>This link expires in 24 hours. If you didn't request a password reset, you can safely ignore this email.</p>
            <div class='footer'>
                <p>&copy; 2025 Asafarim. All rights reserved.</p>
            </div>
        </div>
