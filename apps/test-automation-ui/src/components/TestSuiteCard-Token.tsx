...
export const TestSuiteCardToken: React.FC<TestSuiteCardProps> = ({
  ...
  return (
    <div
      className={`suite-card-token ${isSelected ? "selected" : ""}`}
    ...
    >
      ...
      <div className="suite-card-token__header">
        <div className="suite-card-token__header-left">
          {onToggleSelection && (
            <label
              className="suite-card-token__checkbox"
          ...
          <div className="suite-card-token__info">
            ...
            {suite.fixture && (
              <span className="suite-card-token__fixture">
                <span className="suite-card-token__fixture-label">
                  Fixture:
                </span>{" "}
                {suite.fixture}
              </span>
        ...
        <div className="suite-card-token__header-right">
          <div
            className={`suite-card-token__status-badge ${statusBadge.className}`}
          >
            {statusBadge.icon}
            <span>{statusBadge.label}</span>
          </div>
          <p className="suite-card-token__last-run">
      ...
      <div className="suite-card-token__stats">
        <div className="suite-card-token__stat suite-card-token__stat--total">
          <span className="suite-card-token__stat-value">
            {suite.totalTests}
          </span>
      ...
      {isExpanded && (
        <div
          className="suite-card-token__test-cases"
            ...
            {suite.testCases.length === 0 ? (
              ...
              suite.testCases.map((testCase, index) => {
                ...
                return (
                  <div
                    key={testCase.id}
                    className={`suite-card-token__test-case ${
                      index % 2 === 0
                        ? "suite-card-token__test-case--even"
                        : "suite-card-token__test-case--odd"
                    }`}
                    data-testid={`test-case-${testCase.id}`}
                  >
                    <div className="suite-card-token__test-case-info">
                      <span className="suite-card-token__test-case-name">
                        {testCase.name}
                      </span>
                      {testCase.updatedAt && (
                    ...
                    <div className="suite-card-token__test-case-right">
                      {testCase.duration && (
                        <span className="suite-card-token__test-case-duration">
                          {testCase.duration}ms
                        </span>
                      )}
                      <div
                        className={`suite-card-token__status-badge suite-card-token__status-badge--sm ${caseBadge.className}`}
                      >
                        {caseBadge.icon}
