...
export default function FunctionalRequirementsPage() {
    ...
    if (!fixturesCache[functionalRequirementId]) {
      try {
        const response = await api.get(
          `/api/fixtures?functionalRequirementId=${functionalRequirementId}`
        );
        setFixturesCache((prevCache) => ({
          ...prevCache,
          [functionalRequirementId]: response.data,
        }));
      } catch (error) {
        console.error(error);
      }
  ...
  const renderExpandedRow = (fr: FunctionalRequirement) => {
    ...
    if (isLoading) {
      api
        .get(`/api/fixtures?functionalRequirementId=${fr.id}`)
        .then((response) => {
