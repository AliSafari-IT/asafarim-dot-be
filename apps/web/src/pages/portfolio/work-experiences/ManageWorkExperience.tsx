...
const ManageWorkExperience: React.FC = () => {
  ...
  useEffect(() => {
    const loadWorkExperiences = async () => {
      try {
        ...
        const userIds = new Set(data.filter(exp => exp.userId).map(exp => exp.userId as string));
  ...
  const handleDelete = async (id: number, experienceUserId?: string) => {
    ...
    if (window.confirm('Are you sure you want to delete this work experience?')) {
      setIsDeleting(true);
      try {
        const success = await deleteWorkExperience(id, isAdmin && experienceUserId !== user?.id);
        if (success) {
          // Remove from state
  ...
  if (!isAuthenticated) {
    return <div className="manage-work-experiences">Redirecting to login...</div>;
  }
    ...
    return (
      <div className="manage-work-experiences">
        <div className="manage-work-experiences-container">
          <h1 className="manage-work-experiences-title">Manage Your Work Experience</h1>
      ...
      <div className="manage-work-experiences-container">
        <div className="manage-header">
          <h1 className="manage-work-experiences-title">
            {isAdmin ? 'Manage All Work Experiences' : 'Manage Your Work Experience'}
          </h1>
          <button 
              ...
              <tbody>
                {workExperiences.map((exp) => (
                  <tr key={exp.id}>
                    <td className="table-cell">
                      <div className="table-cell-content">
                        <div className="table-title-wrapper">
                          <Eye onClick={() => handleView(exp.id, exp.userId as string)} className="table-icon"/>
                    ...
                    <td className="table-cell">
                      {exp.isCurrent && <span className="current-indicator">Current</span>}
                    </td>
                    {isAdmin && (
                      <td className="table-cell">
                        <div className="table-text">
                          {exp.userId && userCache[exp.userId] 
                            ? userCache[exp.userId].userName || userCache[exp.userId].email 
                            : 'Unknown user'}
                        </div>
                      </td>
                    )}
                    <td className="table-cell">
                      <div className="table-actions">
                        <button
                          onClick={() => handleView(exp.id, exp.userId as string)}
                        ...
                        <button
                          ...
                          disabled={isDeleting || (!isAdmin && user?.id !== exp.userId)}
