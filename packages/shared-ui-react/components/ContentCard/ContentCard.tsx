...
const ContentCard = ({
  ...
  const viewDocumentUrl = hasViewRoute
    ? `/portfolio/${contentType}/view/${props.id}`
    : undefined;
    ...
    <>
      {showImage && (imageUrl || useGradient) && (
        <div className="content-card__media">
          ...
          {featured && (
            <span className="content-card__featured-badge">Featured</span>
          )}
        </div>
      )}

      {headerContent && (
        <div className="content-card__header">{headerContent}</div>
      )}
      ...
      <div className="content-card__content">
        ...
        {hasViewRoute && (
          <div className="content-card__view-action">
            <a href={viewDocumentUrl} className="content-card__view-button">
              View Document
            </a>
          </div>
        )}
          ...
          <div className="content-card__metrics">
            {metrics.map((metric, index) => (
              <div key={index} className="content-card__metric">
                <span className="content-card__metric-value">
                  {metric.value}
                </span>
                <span className="content-card__metric-label">
                  {metric.label}
                </span>
              </div>
            ))}
        ...
        {actionButton && !clickable && (
          <div className="content-card__actions">{actionButton}</div>
        )}
      </div>

      {footerContent && (
        <div className="content-card__footer">{footerContent}</div>
      )}
    </>
  );
