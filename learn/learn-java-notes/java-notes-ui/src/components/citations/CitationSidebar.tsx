...
import { getCitableNotes } from "../../api/notesApi";
import type {
  CitedNote,
  CitingNote,
  CitationStats,
...
export default function CitationSidebar({
  ...
  const [activeTab, setActiveTab] = useState<"cited" | "citing" | "add">(
    "cited"
  );
  ...
  const loadAllNotes = useCallback(async () => {
    try {
      // Use citable notes endpoint - returns PUBLIC/FEATURED from all users + own notes
      const response = await getCitableNotes({ size: 100 });
      setAllNotes(
        response.items
      ...
      console.error("Failed to load citable notes");
    }
  }, [noteId]);
  ...
  return (
    <>
      ...
      >
        📚{" "}
        {citedNotes.length > 0 && (
          <span className="badge">{citedNotes.length}</span>
        )}
      </button>
      ...
      <div className={`citation-sidebar ${isOpen ? "open" : ""}`}>
        ...
        <div className="sidebar-style-select">
          ...
          >
            {Object.entries(CitationStyleLabels).map(
              ([key, { name, example }]) => (
                ...
                <div key={note.citationId} className="citation-item">
                  <div className="citation-info">
                    ...
                    <div className="citation-details">
                      ...
                      {note.authors && (
                        <span className="citation-author">{note.authors}</span>
                      )}
                    </div>
                  </div>
                ...
                filteredNotes.slice(0, 20).map((note) => (
                  <div key={note.id} className="citation-item add">
                    ...
                    <Button
                      size="sm"
                      variant="secondary"
                      onClick={() => handleAddCitation(note.id)}
                    >
                      + Add
                    </Button>
                  </div>
